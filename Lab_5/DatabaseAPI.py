...
config.read('C:\Фотографії\Lab_14/products_db.conf')
...
class Products(mysql.Model):
    ...
    colour = mysql.Column(mysql.String, nullable=False)
    amount = mysql.Column(mysql.Integer, nullable=False)

    def __repr__(self):
        return '<Products (%s, %s, %s, %s) >' % (self.rate, self.name, self.price, self.colour, self.amount)
...
def create_product():
    ...
    product = Products(id=id, rate=rate, name=name, price=price, colour=colour, amount=amount)
    ...
    result = [data.name, data.rate, data.price, data.colour, data.amount]
...
def get_product():
    ...
    for product in data:
        data_all.append([product.id, product.colour, product.name, product.rate, product.price, product.amount])
